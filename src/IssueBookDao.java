...
public static int save(String bookcallno,int studentid,String studentname,String studentcontact){
	...
	try{
		...
		if(status>0){
		/*PreparedStatement ps=con.prepareStatement("insert into issuebooks(id, bookcallno,studentid,studentname,studentcontact, issueddate) values(?,?,?,?,?,?)");
		ps.setString(1, "A4");
                ps.setString(2,bookcallno);
		ps.setInt(3,studentid);
		ps.setString(4,studentname);
		ps.setString(5,studentcontact);
                ps.setString(6, "20-2-2022");
...
*/
		...
		ps.setString(4,studentcontact);
               // ps.setString(5, "20-2-2022");
		status=ps.executeUpdate();
               
		}
		
		con.close();
	}catch(Exception e){System.out.println(e + "Hie");}
	return status;
}
